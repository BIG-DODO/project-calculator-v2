...
3. 若存在分层原始层数降级到 `<= 4` 层：
   ...
   - 面积段合并（取消双拼，还原原始值）：
     - 对 `originalLayerAreas` 中每个 `area`：
       - 若 `area == 600`：`convertedAreas.push(300)`（600=300×2双拼，还原为300）
       - 若 `area == 800`：`convertedAreas.push(400)`（800=400×2双拼，还原为400）
...
1. 更新 `productOptions`：
   - 若 `selectedProducts.has('layer')`：`productOptions['layer'].floors = layerFloors`
...
1. **前置检查**：
   - 若 `totalAncillary < 1200 + 1600`（配套楼最小2层+宿舍最小2层）：
...
function convertToDuplex(counts, configs):
   ...
   for i from 0 to configs.length - 1:
      ...
      // 分层≤800强制双拼的，跳过转换（已经是双拼基底）
      if c.id == 'layer' and c.base <= 800:
         output.push({
            type: c.type,
            base: c.base,  // 已是双拼后的基底（如600→1200）
            floors: c.floors,
            count: count,  // 保持原栋数
            isDuplex: true,
            note: '强制双拼'
      ...
      // 其他类型：偶数栋→全双拼，奇数栋→双拼+1独栋
      if count % 2 == 0:
         // 偶数栋：全部转为双拼
         duplexCount = count / 2
         output.push({
            type: c.type,
            base: c.base * 2,  // 双拼基底 = 独栋基底×2
            floors: c.floors,
            count: duplexCount,
            isDuplex: true,
            note: '双拼'
...
function checkDistribution(counts, configs):
   ...
   for each type in ['light-steel', 'split', 'layer']:
      ...
      if group.indices.length === 0: continue  // 空数组保护
...
// 振荡检测：记录最近3轮的密度偏差方向
oscillationHistory = []  // 存储最近3轮的密度偏差符号（+1: 超上限, -1: 低于下限, 0: 正常）
